...
public class TestAlbumImpl {
  ...
  public void testAddShape() {
    ...
    assertEquals("[Name: R1\nType: rect\nMin corner: (50, 200), "
                    + "Width: 25, Height: 200, Color: (20,30,200), "
                    + "Name: R2\nType: rect\nMin corner: (100, 400), "
                    + "Width: 50, Height: 100, Color: (0,80,15), "
                    + "Name: R3\nType: rect\nMin corner: (200, 75), "
                    + "Width: 75, Height: 20, Color: (150,50,0)]"
    ...
    assertEquals("[Name: Oval1\n"
                    + "Type: oval\n"
                    + "Center: (600, 300), Width: 100, Height: 50, Color: (0,50,200), "
                    + "Name: Oval2\n"
                    + "Type: oval\n"
                    + "Center: (444, 555), Width: 200, Height: 65, Color: (0,220,10)]",
  ...
  public void testRemoveShape() {
    ...
    assertEquals("[Name: R1\n"
                    + "Type: rect\n"
                    + "Min corner: (50, 200), Width: 25, Height: 200, Color: (20,30,200)]",
            album1.getShapes().values().toString());
    ...
    assertEquals("[Name: R3\n"
                    + "Type: rect\n"
                    + "Min corner: (200, 75), Width: 75, Height: 20, Color: (150,50,0)]",
  ...
  public void testGetShapes() {
    assertEquals("[Name: R1\n"
                    ...
                    + "Name: R2\n"
                    + "Type: rect\n"
                    + "Min corner: (100, 400), Width: 50, Height: 100, Color: (0,80,15)]",
            album1.getShapes().values().toString());
  ...
  public void testAddShapshotAndToString2() {
    ...
    // GENERAL TESTING FOR CONTROLLER
    Map snapshots = album3.getSnapshots();
    List<Snapshot> keyList = new ArrayList<>(snapshots.keySet());

    List<Shape> shapes = new ArrayList<>(snapshots.values());

    // GET LIST OF SNAPSHOT IDS
    List<String> snapshotIdList = keyList.stream().map(s -> s.getSnapshotId()).collect(Collectors.toList());

    // GET LIST OF SNAPSHOT DESCRIPTIONS
    //List<String> snapshotDescriptionList = keyList.stream().map(s -> s.getDescription()).collect(Collectors.toList());
    //assertEquals("[snapshot1, snapshot2, Snapshot 3]", snapshotDescriptionList);
