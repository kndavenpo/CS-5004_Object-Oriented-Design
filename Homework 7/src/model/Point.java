...
public class Point {
  ...
  public Point(int x, int y) throws IllegalArgumentException {
    if (x <= 0 || x >= pageWidth) {
      throw new IllegalArgumentException("The X must be greater than 0 and less than or equal to "
              + "the page width.");
    }
    if (y <= 0 || y >= pageHeight) {
      throw new IllegalArgumentException("The Y must be greater than 0 and less than or equal to "
              + "the page height.");
    }
    this.pointX = x;
    this.pointY = y;
